...
class AuthControllerTest {
  ...
  void register_shouldReturnCreatedAndDelegateToService() throws Exception {
    ...
    mockMvc
        .perform(
            post(APIConstant.API_AUTH_PATH + "/register")
  ...
  void login_shouldReturnAuthResponseAndSetRefreshCookie() throws Exception {
    ...
    ResponseCookie refreshCookie =
        ResponseCookie.from(
                APIConstant.REFRESH_TOKEN_NAME, "asfsafwaruiwur192412u412irnfsakfbiwahfawf")
            .httpOnly(true)
    ...
    mockMvc
        .perform(
            post(APIConstant.API_AUTH_PATH + "/login")
                .contentType(MediaType.APPLICATION_JSON)
                .content(json))
  ...
  void logout_shouldReturnNoContentAndSetExpiredCookie() throws Exception {
    ResponseCookie cookie =
        ResponseCookie.from(APIConstant.REFRESH_TOKEN_NAME, "")
            .maxAge(0)
            .path(APIConstant.API_AUTH_PATH)
            .httpOnly(true)
            .build();
    ...
    mockMvc.perform(post(APIConstant.API_AUTH_PATH + "/logout")).andExpect(status().isNoContent());
  ...
  void refreshToken_shouldUseCookieAndReturnNewAccessToken() throws Exception {
    ...
    mockMvc
        .perform(
            post(APIConstant.API_AUTH_PATH + "/refresh-token")
                .cookie(new Cookie(APIConstant.REFRESH_TOKEN_NAME, refreshToken)))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.accessToken").value("asfsafwaruiwur192412u412irnfsakfbiwahfawf"));
